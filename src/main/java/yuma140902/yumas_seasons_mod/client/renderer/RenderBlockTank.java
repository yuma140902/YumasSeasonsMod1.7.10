...
public class RenderBlockTank implements ISimpleBlockRenderingHandler {
	...
	public void renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks renderer) {
		...
		if(iconTop == null) {
			iconTop = MyBlocks.tank.getIcon(Consts.SIDE_TOP, metadata);
		}
		if(iconSide == null) {
			iconSide = MyBlocks.tank.getIcon(Consts.SIDE_NORTH, metadata);
		}
	...
	public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
		...
		{
			if(iconTop == null) {
				iconTop = MyBlocks.tank.getIcon(Consts.SIDE_TOP, 0);
			}
			if(iconSide == null) {
				iconSide = MyBlocks.tank.getIcon(Consts.SIDE_NORTH, 0);
			}
