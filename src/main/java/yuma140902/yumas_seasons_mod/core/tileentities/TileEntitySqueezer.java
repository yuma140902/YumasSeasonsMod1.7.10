package yuma140902.yumas_seasons_mod.core.tileentities;
...
public class TileEntitySqueezer extends TileEntity implements IInventory, IFluidTankContainer, ITileEntityDropable {
	...
	private void startProcessingIfNecessary() {
		...
		FluidStack product = CoreRecipes.squeezer.getProduct(inputSlot);
		if(product == null) {
			return;
		}
	...
	private void processTick() {
		...
		if(procTick >= MAX_PROC_TICK) {
			this.fill(ForgeDirection.UNKNOWN, CoreRecipes.squeezer.getProduct(inputSlot), true);
			procTick = 0;
